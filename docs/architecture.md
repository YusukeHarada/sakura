...
|---|---|
| 文書種別 | アーキテクチャ設計書 |
| 対象 | サクラエディタ本体（`sakura_core`） |
| 関連文書 | [要求仕様書](requirements.md) / [コンポーネント設計書](component-design.md) |
...
各サブシステムの内部構造は [コンポーネント設計書](component-design.md) が扱います。

- 対象コミット時点の `sakura_core/` 以下の実装に基づきます
- ビルド手順は [`tools/build.md`](../tools/build.md)、開発の進め方は [`CONTRIBUTING.md`](../CONTRIBUTING.md) を参照してください

> **本書の性質について**
> サクラエディタには当初のアーキテクチャ設計書が存在しません。
> 本書は実装から読み取った構造の記述であり、
> 「設計判断」の章に挙げた理由のうちコード上に根拠が無いものは
> **推定であることを明示**しています。
...
実装から読み取れる、構造を決定づけている要因です。
対応する品質要求 ID は [要求仕様書](requirements.md#4-品質要求) の QR-xx に対応します。

| # | 主要因 | 影響した構造 | 対応 QR |
|---|---|---|---|
| D-1 | 複数ウィンドウで設定と履歴を一貫させたい | 二プロセスモデル + 共有メモリ | QR-04 |
| D-2 | 1 ウィンドウのクラッシュを他へ波及させたくない（推定） | エディタプロセスの分離 | — |
| D-3 | 折り返し表示と論理行を両立させたい | 論理行 / レイアウト行の二層構造 | QR-02 |
| D-4 | 2 つの座標系の取り違えを防ぎたい | `CStrictInteger` による型分離 | QR-05 |
| D-5 | 大量の機能をメニュー・キー・マクロから一様に呼びたい | `EFunctionCode` への一元化 | QR-07 |
| D-6 | 多様な文字コードを扱いたい | 内部 UTF-16 統一 + `CCodeBase` の抽象化 | — |
| D-7 | 第三者が機能を追加できるようにしたい | ジャック（フック）機構 | QR-07 |
| D-8 | 描画コストを抑えたい | 画面キャッシュ、文字幅キャッシュ | QR-02 |
| D-9 | 過去バージョンとの互換を壊したくない | 機能コード体系の固定、設定既定値の維持 | QR-01 |

**D-2 は推定です。** プロセス分離の理由を述べたコメントはコード上に見当たりません。
...
| `29001`〜 | Main Menu 特殊機能（動的リスト） |
| `30000`〜`32767` | メニュー・キーに割り当て可能な機能番号 |
| `32768`〜`39999` | メニューから直接呼ばれず、他 ID から間接的に呼ばれる機能 |
| `40000`〜`49511` | マクロ関数 |
| `49512`〜 | 変換コマンド（状況に応じて機能を置き換える） |

機能番号は 16bit の数値です。`HandleCommand` の引数としては 32bit 幅ですが、
**上位 16bit はコマンドが送られた状況の通知に使われます**
（`FA_FROMMACRO` = マクロからの実行、`FA_NONRECORD` = マクロへの記録を抑制）。

> **`32767` という上限は Windows 95 に由来します。**
> 「Windows 95 では 32768 以上のコマンドをメニューやアクセラレータに設定すると
> 動作しない」と `Funccode_x.hsrc` に明記されています。
> 現在サポート対象外の OS の制約が、互換性維持のために体系として残っている例です。
...
主要な構造上の選択について、判明している範囲で理由と帰結を整理します。
**根拠欄が「推定」のものはコード上に記録が無く、実装から逆算した解釈です。**

### AD-1: 二プロセスモデル

| | |
|---|---|
| **選択** | 共有状態を持つコントロールプロセスを 1 つ立て、編集ウィンドウは別プロセスにする |
| **根拠** | 複数ウィンドウ間で設定・履歴を一貫させるため（D-1）。プロセス分離による耐障害性は**推定** |
| **代替案** | 単一プロセス + 複数ウィンドウ（一般的な MDI/SDI 構成） |
| **帰結（正）** | ウィンドウ単位で独立。設定は常に一元管理される |
| **帰結（負）** | IPC が必須になり共有メモリと `MYWM_*` メッセージの複雑さを負う。`DLLSHAREDATA` のレイアウト変更時は全プロセスの再起動が必要 |
| **制約** | Windows 固有 API に強く依存し、移植の障壁になっている |

起動競合について、コードは**あえて厳密な排他を行っていません**。
`IsExistControlProcess()` はコントロールプロセスがミューテックスを確保し終えるまで
false を返すため、同時起動時にコントロールプロセスが複数生まれ得ます。
最初にミューテックスを取った 1 つだけが生き残る設計で、
「起動を直列化するより、競合しても結果が正しくなるようにする」という判断です
（`_main/CProcessFactory.cpp` のコメントに明記）。

### AD-2: 論理行とレイアウト行の二層化

| | |
|---|---|
| **選択** | ファイル上の行（`CDocLineMgr`）と画面上の行（`CLayoutMgr`）を別の層にする |
| **根拠** | 折り返し・タブ展開を表示側だけの関心事にするため（D-3） |
| **代替案** | 表示のたびに折り返しを計算する |
| **帰結（正）** | 編集は論理行に対して行えばよく、表示都合が編集ロジックに混入しない |
| **帰結（負）** | 編集のたびにレイアウトの再計算が必要。2 つの座標系が全体に波及する |

この「2 つの座標系」の副作用への対処が AD-3 です。

### AD-3: 座標系の型分離（`CStrictInteger`）

| | |
|---|---|
| **選択** | `CLogicInt` と `CLayoutInt` をタグ引数で別型にし、混用をコンパイルエラーにする |
| **根拠** | AD-2 が生んだ取り違えリスクを型で潰すため（D-4） |
| **代替案** | どちらも `int` にして規約と命名で運用する |
| **帰結（正）** | 座標系の取り違えがビルド時に露見する |
| **帰結（負）** | **MSVC の Debug ビルドでしか有効にならない**（`config/build_config.h`）。リリースビルドでは単なる `int` に落ちるため、Debug を通さないと恩恵が無い |

`CLayoutInt` のほうが `int` への暗黙変換を禁止しており、より厳格です。
非対称にした理由はコード上に記録がなく**不明**です。

### AD-4: 機能の `EFunctionCode` への一元化

| | |
|---|---|
| **選択** | すべての操作を 1 つの列挙で表し、`CViewCommander::HandleCommand()` に集約する |
| **根拠** | メニュー・キーバインド・ツールバー・マクロ・プラグインから同じ操作を一様に呼ぶため（D-5） |
| **代替案** | 呼び出し経路ごとに個別のインタフェースを設ける |
| **帰結（正）** | キー割り当てとマクロが機能追加に自動的に追随する |
| **帰結（負）** | 巨大な分岐になり、`CViewCommander_*.cpp` へのファイル分割で緩和している。番号体系が歴史的制約（AD-6）を抱える |

定義は `Funccode_x.hsrc` を単一の情報源とし、ヘッダは生成物です。
**機能追加時に `.hsrc` 以外を編集してはいけません。**

### AD-5: 文字コードの内部 UTF-16 統一

| | |
|---|---|
| **選択** | 内部表現を UTF-16（`CNativeW`）に固定し、入出力の境界でのみ変換する |
| **根拠** | 多様な文字コードを扱いつつ、編集ロジックを 1 種類に保つため（D-6）。Win32 API が UTF-16 前提であることも整合する |
| **代替案** | UTF-8 内部統一、あるいは元の文字コードのまま保持 |
| **帰結（正）** | 新しい文字コードの追加が `CCodeBase` の派生 + `CCodeFactory` への 1 行で済む |
| **帰結（負）** | UTF-8 ファイルの読み書きで必ず変換コストがかかる。サロゲートペアの扱いが別途必要 |

### AD-6: 後方互換性の優先

| | |
|---|---|
| **選択** | 過去の制約を、根拠が失効した後も維持する |
| **根拠** | 既存ユーザーの設定・マクロ・プラグインを壊さないため（D-9） |
| **具体例** | 機能番号の上限 `32767`（Windows 95 由来）。`Funccode_x.hsrc` に理由が明記されている |
| **帰結（正）** | 長期間の設定資産が生き続ける |
| **帰結（負）** | 現代の環境では不要な制約がコード体系に残り続ける |

### AD-7: 外部ライブラリへの委譲

正規表現・Migemo・タグ・差分は自前実装せず外部に委譲しています
（`extmodule/` がラッパ）。中核の編集機能に集中する判断と読めますが、
**明示的な根拠はコード上にありません（推定）**。

帰結として、これらの機能は外部モジュールの有無で可用性が変わります。

---

## 18. 既知のアーキテクチャ上の負債

改善を検討する際の出発点として、構造上の弱い箇所を挙げます。

| # | 内容 | 影響 |
|---|---|---|
| T-1 | UI 層が抽象化されておらず、`HWND` と GDI が文書モデル近くまで貫通している | 描画方式の変更や移植が困難。`sakura_core` の 696 ファイル中 239 が Win32 型に直接依存 |
| T-2 | DPI 情報がプロセスグローバル（`CDPI`）で、`WM_DPICHANGED` が未処理 | 複数モニタで DPI が異なる環境に追随できない |
| T-3 | `CStrictInteger` による型検査が Debug ビルド限定 | リリースビルドのみで開発すると座標系の誤りを検出できない |
| T-4 | ダイアログが 69 個すべて Win32 リソース定義 | UI 刷新のコストが高い |
| T-5 | MSVC 側のソース一覧が `vcxproj` に明示列挙（CMake 側は glob） | 追加漏れがビルドエラーになる二重管理 |
